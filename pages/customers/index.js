...
function CustomersPage({ customers = [] }) {
  ...
  return (
    <Layout>
      <div>
        <h2 className="text-2xl font-bold tracking-tight text-gray-900 dark:text-white mb-4">Customer List</h2>
        <div className="flex justify-end">
          <button
            className="bg-light-500 hover:shadow-xl ml-2 py-2 px-5 rounded shadow-md mb-4"
            onClick={() => {
              let temp = Object.assign([], localCustomers);
              temp.push({
                customerId: 0,
                firstName: "",
                lastName: "",
                email: "",
                id: -2 // this means new customer
              });
              setLocalCustomers(temp)
            }}
          >
            Add New Cusomter
          </button>
